...
GLfloat light_diffuse[] = { 1, 1, 1, 1 };//0.9, 1, 1, 1
GLfloat light_specular[] = { 1, 1, 1, 1 };
GLfloat light_cloudposition[] = { -0.8, 3.2, 1.0, 1 }; // -0.1, 90, -1.0, 1.0
GLfloat cloudshininess = 30;
...
GLfloat keyFrameTimes[] = {0, 18, 22, 35, 45, 55}; // t starting values of keyframes 
...
void init() {
    ...
    myTexture4 = LoadTexture("lake.bmp", 1200, 800); //lake      
...
void waterfall() {  
    ...
    float A[3] = { -0.6, 0.251,  -0.5 };
    float B[3] = { -0.4, 0.251,  -0.5 };
...
void clouds(void) {  
    ...
    glLightfv(GL_LIGHT1, GL_AMBIENT, light_ambient);
    glLightfv(GL_LIGHT1, GL_DIFFUSE, light_diffuse);
    glMateriali(GL_FRONT_AND_BACK, GL_SHININESS, cloudshininess);
...
void evaporate(void) {    
    ...
    // it reached the cloud so disappear inside the ground
    if (smove >= -0.2)
        smove = -0.95;
    
    // it`s still in its keyframe time, so make the animation
    if (t < keyFrameTimes[1])
        smove += 0.0004;          
}
...
void condense(void) {
    ...
    if (t > keyFrameTimes[1] && t < keyFrameTimes[2]) {
        ...
        light_diffuse[0] = light_diffuse[1] = light_diffuse[2] = 0.2;
        cloudshininess = 30;       
...
void display() {
    ...
    //scaleAndTranslate(-0.45, 0, -0.4, 0.45, tree);
